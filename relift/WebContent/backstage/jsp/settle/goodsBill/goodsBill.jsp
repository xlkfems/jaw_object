...
<body>
    <div class="layui-tab layui-tab-brief" lay-filter="demo">
        <ul class="layui-tab-title">
            <li class="layui-this">新结运单</li>
        </ul>
        <br/>&nbsp;
      <div class="layui-input-inline">
      <form action="<%=basePath%>centreServlet?oper=query_phon" method="post">
		  ...
		  <tr>
    	   ...
    	   <c:forEach items="${pages.getListPlace()}" var="place">
    	    <tr>
    	       ...
    	       <td>${place.money}入</td>
    	       <td>
    	       <button type="button" class="layui-btn layui-btn-sm" onclick="close_goodsBill('结算','<%=basePath%>placeServlet?oper=query_place&number=${place.number}','800','500')">结算</button>
    	       </td>
    	   </tr>
    	   </c:forEach>
    	</table>
    	 <center>
    	<a href="<%=basePath%>centreServlet?oper=query_place&pageNo=${1}"><button type="button" class="layui-btn  layui-btn-sm">首页</button></a>
 		<a href="<%=basePath%>centreServlet?oper=query_place&pageNo=${pages.currPageNo-1}"><button type="button" class="layui-btn   layui-btn-sm">上一页</button></a>
 		<button type="button" class=" layui-btn-primary layui-btn-sm">${pages.getCurrPageNo()}</button>
 		<a href="<%=basePath%>centreServlet?oper=query_place&pageNo=${pages.currPageNo+1}"><button type="button" class="layui-btn  layui-btn-sm">下一页</button></a>
 		<a href="<%=basePath%>centreServlet?oper=query_place&pageNo=${pages.currPageNo+pages.getTotalPageCount()}"><button type="button" class="layui-btn  layui-btn-sm">尾页</button></a>
 		</center>
            </div>
